...
import { APP_SEED, models, txBasic, txOther } from './common'
...
describe('Standard', function () {
  ...
  test.each(models)('get address', async function (m) {
    ...
    try {
      ...
      const expected_address = '4iaZq9SR9sjK6YU7a1s1MGy1J6AujG2GrMCASMFHkPgbGk9q'
      const expected_pk = '402fce82def04d8d355b0cbbf715fe72db6e46e6b2ddcac7a190fe01606e6295'
  ...
  test.each(models)('sign other', async function (m) {
    const sim = new Zemu(m.path)
    try {
      await sim.start({ ...defaultOptions, model: m.name })
      const app = newNodleApp(sim.getTransport())
      ...
      await sim.compareSnapshotsAndAccept('.', `${m.prefix.toLowerCase()}-sign_large_other`, m.name === 'nanos' ? 7 : 8)
