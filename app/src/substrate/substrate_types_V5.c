...
parser_error_t _readOptionCollectionId_V5(parser_context_t* c, pd_OptionCollectionId_V5_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readCollectionId_V5(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionItemId_V5(parser_context_t* c, pd_OptionItemId_V5_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readItemId_V5(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionItemPrice_V5(parser_context_t* c, pd_OptionItemPrice_V5_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readItemPrice_V5(c, &v->contained))
    }
    return parser_ok;
}
    ...
    uint8_t* pageCount)
{
    ...
    // First measure number of pages
    uint8_t pages[2] = { 0 };
    CHECK_ERROR(_toStringAccountId_V5(&v->accountid, outValue, outValueLen, 0, &pages[0]))
    ...
    *pageCount = 0;
    ...
    if (pageIdx < pages[0]) {
        CHECK_ERROR(_toStringAccountId_V5(&v->accountid, outValue, outValueLen, pageIdx, &pages[0]))
        return parser_ok;
    }
    pageIdx -= pages[0];
...
parser_error_t _toStringVecItemId_V5(
    ...
    uint8_t* pageCount)
{
    GEN_DEF_TOSTRING_VECTOR(ItemId_V5);
}
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringCollectionId_V5(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringItemId_V5(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
